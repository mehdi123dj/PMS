...
from model import double_seq_LSTM
...
def train(model,
    ...
    for epoch in range(num_epochs):
        for it in train_loader:    
            ...
            P = P.to(device)
            NA = NA.to(device)
            Lens_f = Lens_f.to(device)
            Lens_t = Lens_t.to(device)

            output = model(text, G, S, P,NA,Lens_f.cpu(),Lens_t.cpu())
            ...
            if global_step % eval_every == 0:
                ...
                with torch.no_grad():                    
                    ...
                    for itv in valid_loader:    
                        ...
                        Lens_vf = G_len_v + S_len_v + P_len_v + NA_len_v
                        Lens_vt = text_len_v
                        output_v = model(text_v, G_v, S_v, P_v,NA_v,Lens_vf.cpu(),Lens_vt.cpu())
...
def main():
    ...
    model = double_seq_LSTM(vocab_size = len(TEXT.vocab),
